import os, sys
sys.path.append('F:\printcollorpi')
os.environ['DJANGO_SETTINGS_MODULE'] = 'project.settings'
import django
django.setup()
from api.models import (Empresa, Usuario, Cliente, Produto, Orcamento,
...
# Clientes
for nome, tel, email in [('Joao Silva', '11988881111', 'joao@email.com'), ('Maria Santos', '11988882222', 'maria@email.com'), ('Pedro Costa', '11988883333', 'pedro@email.com')]:
    Cliente.objects.get_or_create(nome=nome, defaults={'telefone': tel, 'email': email, 'empresa': e1})
print('Clientes:', Cliente.objects.count())

# Produtos
for nome, un, preco, est in [('Adesivo Vinil', 'Folha', 5.00, 150), ('DTF A4', 'Unidade', 8.50, 500), ('DTF A3', 'Unidade', 15.00, 300)]:
    Produto.objects.get_or_create(nome=nome, defaults={'unidade': un, 'preco': preco, 'estoque': est})
