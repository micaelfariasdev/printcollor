      ...
      <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8">
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <div
              className={`w-10 h-6 rounded-full relative transition-colors ${isAdmin ? 'bg-amber-400' : 'bg-slate-200'}`}
            >
              <div
                className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isAdmin ? 'left-5' : 'left-1'}`}
              />
            </div>
          </button>
        </div>
        ...
        <div className="flex gap-3 mt-8">
          <button onClick={onClose} className="flex-1 font-bold text-slate-400">
            Cancelar
          </button>
          <button
            onClick={handleSave}
            disabled={loading}
            className={`${theme.colors.primaryButton} flex-[2] text-white py-4 rounded-2xl font-black uppercase shadow-xl disabled:opacity-50`}
          >
            {loading ? (
