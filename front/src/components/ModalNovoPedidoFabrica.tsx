    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 text-left">
      <div className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in fade-in zoom-in duration-300">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5 overflow-y-auto">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">
              Nome do Pedido / Descrição Curta
            </label>
            <input
              required
              placeholder="Ex: Uniforme Escolar 2026"
              className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold"
              value={formData.nome_descricao}
              onChange={(e) =>
                setFormData({ ...formData, nome_descricao: e.target.value })
              }
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, aplicacao_arte: e.target.value })
                }
              />
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black hover:bg-blue-700 transition"
              >
                <Shirt size={14} /> CAMISARIA
              </button>
              <button
                type="button"
                onClick={() => aplicarPreset('calcas')}
                className="flex items-center gap-2 px-3 py-2 bg-slate-800 text-white rounded-xl text-[10px] font-black hover:bg-black transition"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-bold hover:bg-slate-50 transition"
              >
                📦 QUANTIDADE ÚNICA
              </button>
            </div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-7 gap-2 bg-slate-50 p-3 rounded-2xl border border-slate-100">
            {Object.entries(formData.detalhes_tamanho).map(
              ([chave, valor]: any) => (
                <div
                  key={chave}
                  className="bg-white p-2 rounded-xl border border-slate-100 text-center relative group"
                >
                  <span className="text-[12px] font-black text-slate-400 uppercase block mb-1">
                    {chave}
                  </span>
                  <input
                    type="number"
                    value={valor}
                    className="w-full text-center font-black text-blue-600 outline-none text-xs bg-transparent"
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        detalhes_tamanho: {
            ...
            <div className="pointer-events-none">
              {arquivo ? (
                ...
                <div className="text-slate-400">
                  <Upload
                    size={32}
                    className={`mx-auto mb-2 text-blue-500 ${isDragging ? 'animate-bounce' : ''}`}
                  />
                  <p className="text-[10px] font-black uppercase tracking-tighter">
                    Arraste ou Ctrl+V para o Layout
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-[2] bg-blue-600 text-white py-4 rounded-2xl font-black uppercase shadow-xl shadow-blue-200 flex items-center justify-center gap-2 hover:bg-blue-700 transition active:scale-95"
            >
              {loading ? (
                <Loader2 className="animate-spin" size={20} />
              ) : (
                <>
                  <Save size={20} /> CRIAR PEDIDO
                </>
              )}
            </button>
          </div>
