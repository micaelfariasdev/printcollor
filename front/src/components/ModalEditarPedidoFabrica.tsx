...
export default function ModalEditarPedidoFabrica({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const clienteId =
        typeof formData.cliente === 'object'
          ? formData.cliente.id
          : formData.cliente;
      ...
      addAlert(
        'Alterações salvas! O painel de produção foi atualizado.',
        'success'
      );
      onSuccess();
      onClose();
      ...
      addAlert(
        'Não foi possível atualizar o pedido. Verifique a grade.',
        'error'
      );
    } finally {
      setLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in fade-in zoom-in duration-300">
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-white rounded-full transition active:scale-90"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
            ...
            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Novo (ex: XG)"
                  className="p-1.5 text-[10px] rounded-lg border border-slate-200 outline-none w-24"
                />
                <button
                  onClick={addNovoCampo}
                  className="bg-slate-800 text-white p-1.5 rounded-lg hover:bg-black transition text-[10px] font-bold uppercase"
                >
                  Add
                </button>
              </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
              {Object.entries(formData.detalhes_tamanho).map(
                ([chave, valor]: any) => (
                  ...
                  >
                    <label className="text-[9px] font-black text-slate-400 uppercase block mb-1 text-center">
                      {chave}
                    </label>
                    <input
                      type="number"
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              onChange={handleInputChange}
              className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm h-16"
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1 flex items-center gap-1">
              <ImageIcon size={10} /> Trocar Layout (Opcional)
            </label>
            <div
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-4">
          <button
            onClick={onClose}
            className="flex-1 py-4 font-bold text-slate-400 hover:text-slate-600 transition uppercase text-xs"
          >
            Cancelar
          </button>
          <button
            onClick={handleSave}
            disabled={loading}
            className="flex-[2] py-4 bg-blue-600 text-white font-black rounded-2xl shadow-xl shadow-blue-200 hover:bg-blue-700 transition flex items-center justify-center gap-2 uppercase tracking-widest text-xs"
          >
            {loading ? (
