...
const ModalNovoOrcamento: React.FC<Props> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const adicionarItem = () => {
    setItens([...itens, { produto: '', descricao: '', quantidade: 1, preco_negociado: 0 }]);
  };
  ...
  const salvarOrcamento = async () => {
    if (!selectedCliente || !selectedEmpresa) return alert("Selecione cliente e empresa.");
    ...
    try {
      ...
      onClose();
      // Reset form
      setItens([{ produto: '', descricao: '', quantidade: 1, preco_negociado: 0 }]);
    } catch (err) {
      console.error(err);
      alert("Erro ao salvar. Verifique se todos os itens têm produto selecionado.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white w-full max-w-4xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[95vh] border border-slate-100 overflow-hidden">
        ...
        <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-white">
          <div>
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Painel Administrativo {theme.appName}</p>
          </div>
        ...
        <div className="p-8 overflow-y-auto space-y-8 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-500 uppercase ml-1">Empresa Emissora</label>
              <select 
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-500 uppercase ml-1">Cliente Destino</label>
              <select 
                value={selectedCliente}
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3.5 outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-700 transition-all"
                onChange={(e) => setSelectedCliente(e.target.value)}
              >
                <option value="">Quem é o cliente?</option>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {itens.map((item, index) => (
                <div key={index} className="group relative flex flex-col gap-4 bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 hover:border-blue-200 transition-all">
                  <div className="grid grid-cols-12 gap-4">
                    ...
                    <div className="col-span-4 md:col-span-2 space-y-1">
                      <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Qtd</label>
                      <input 
                        type="number" 
                    ...
                    <div className="col-span-6 md:col-span-3 space-y-1">
                      <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Preço Negociado</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Descrição / Observações Técnicas</label>
                    <input 
                      type="text"
        ...
        <div className="p-8 border-t border-slate-100 flex items-center justify-between bg-white">
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor Total Líquido</span>
          ...
          <div className="flex gap-3">
            ...
            <button 
              ...
              disabled={loading || !selectedCliente || itens.some(i => !i.produto)}
            ...
            >
              {loading ? <Loader2 className="animate-spin" size={20}/> : <><Save size={20}/> Gerar Pedido</>}
            </button>
          </div>
        </div>
      </div>
