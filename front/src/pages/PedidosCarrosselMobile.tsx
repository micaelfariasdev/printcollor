...
import { Layers, Package, Calendar, Maximize2, ChevronLeft, ChevronRight, ArrowLeftCircle, ArrowRightCircle } from 'lucide-react';

export const PedidosCarrosselMobile = () => {
  const [pedidos, setPedidos] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const scrollRef = useRef<HTMLDivElement>(null); // Referência para controlar o scroll via botões
  ...
  if (loading) return <div className="h-screen w-screen flex items-center justify-center bg-slate-950 text-blue-500 font-black italic uppercase text-sm">Carregando Painel...</div>;
