...
interface PaginatedState<T> {
  ...
  refresh: () => void;
  updateItem: (id: number, patch: Partial<T>) => void;
}
...
export function usePaginatedList<T = any>({
  ...
  // Reset + reload when endpoint changes
  useEffect(() => {
    setSearch('');
    setItems([]);
    setOffset(0);
  ...
  const updateItem = useCallback((id: number, patch: Partial<T>) => {
    setItems((prev) => prev.map((item) => ((item as any).id === id ? { ...item, ...patch } : item)));
  }, []);

  const hasMore = items.length < totalCount;

  return { items, loading, hasMore, totalCount, loadMore, setSearch: setSearchDebounced, refresh, updateItem };
}